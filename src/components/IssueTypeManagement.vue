  ...
  setup() {
    ...
    const fetchIssueTypes = async () => {
     ...
     const handleAddIssueType = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('https://vswiftsupport.gov.vc/api/issue-types/create', {
          method: 'POST',
          headers: {
            'Authorization': 'Bearer '+ token,
            'Content-Type': 'application/json'
   ...
   const handleDeleteIssueType = async (issueTypeId) => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://vswiftsupport.gov.vc/api/issue-types/${issueTypeId}`, {
          method: 'DELETE',
           headers: {
            'Authorization': 'Bearer '+ token,
            'Content-Type': 'application/json'
