  ...
  const auth = useAuthStore();
    ...
    const fetchUsers = async () => {
      try {
        ...
        const response = await fetch('https://learnvswift.gov.vc:8080/api/users',
                     {
                    headers: {
    ...
    const handleAddUser = async () => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch('https://learnvswift.gov.vc:8080/api/users/create', {
          method: 'POST',
          headers: {
            'Authorization': 'Bearer '+ token,
            'Content-Type': 'application/json'
    ...
    const handleDeleteUser = async (userId) => {
      ...
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://learnvswift.gov.vc:8080/api/users/${userId}`, {
          method: 'DELETE',
          headers: {
            'Authorization': 'Bearer '+ token,
            'Content-Type': 'application/json'
    ...
    const handleRoleChange = async (user) => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://learnvswift.gov.vc:8080/api/users/edit/role/${user.id}`, {
          method: 'PUT',
          headers: {
            'Authorization': 'Bearer '+token,
            'Content-Type': 'application/json'
    ...
    const handleIssueTypeChange = async (user) => {
     ...
     const handleActiveStatusChange = async (user) => {
      try {
        const token = localStorage.getItem('token');
        const response = await fetch(`https://learnvswift.gov.vc:8080/api/users/edit/activeStatus/${user.id}`, {
          method: 'PUT',
          headers: {
            'Authorization': 'Bearer '+token,
            'Content-Type': 'application/json'
