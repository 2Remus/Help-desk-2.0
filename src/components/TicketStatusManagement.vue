...
import { useAuthStore } from '../stores/auth';
    ...
    const fetchTicketStatuses = async () => {
     ...
     const handleAddTicketStatus = async () => {
      try {
        ...
        const response = await fetch('http://10.181.1.64:8080/api/ticket-status/create', {
          method: 'POST',
          headers: {
            'Authorization': 'Bearer '+ token,
            'Content-Type': 'application/json'
   ...
   const handleDeleteTicketStatus = async (ticketStatusId) => {
      ...
      try {
        const token = auth.token;//localStorage.getItem('token');
        const response = await fetch(`http://10.181.1.64:8080/api/ticket-status/${ticketStatusId}`, {
          method: 'DELETE',
           headers: {
            'Authorization': 'Bearer '+ token,
            'Content-Type': 'application/json'
